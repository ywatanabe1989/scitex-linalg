...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-linalg's —
...
# so only scitex-linalg's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_linalg-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-linalg's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-linalg + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-linalg's historical bare-uv/pip workflow so a
...
# scitex-linalg's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-linalg style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-linalg-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-linalg's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_linalg --cov-report=xml --cov-report=term \
    -p no:cacheprovider
